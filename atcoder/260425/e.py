...
for s in S:
    a_rl.append(a_rl[-1] + (1 if s == "A" else 0))
    b_rl.append(b_rl[-1] + (1 if s == "B" else 0))
    c_rl.append(c_rl[-1] + (1 if s == "C" else 0))

ans = 0

for start_idx in range(1, N+1):
    for end_idx in range(start_idx, N+1):
        a_cnt = a_rl[end_idx] - a_rl[start_idx-1]
        b_cnt = b_rl[end_idx] - b_rl[start_idx-1]
        c_cnt = c_rl[end_idx] - c_rl[start_idx-1]
        # print(start_idx, end_idx, a_cnt, b_cnt, c_cnt)
        if a_cnt != b_cnt and b_cnt != c_cnt and c_cnt != a_cnt:
            ans += 1
