...
while len(q):
    item = q.popleft()
    cur, children, par = item[0], g[item[0]], item[1]
    visited.add(cur)
    # print("始まるよ", cur, setdict[cur], A[cur])
    ans[cur] = (A[cur] in setdict[cur]) or par
    setdict[cur].add(A[cur])
    for c in children:    # 子供
        setdict[c] = setdict[cur].copy()
        if c not in visited:
            q.append((c, ans[cur]))
